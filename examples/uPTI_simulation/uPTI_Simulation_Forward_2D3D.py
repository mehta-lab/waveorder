...
if sample_type == '3D':
    ...
    blur_size = 1 * ps
    target, azimuth, inclination = wo.genStarTarget_3D((N, M, L), ps, psz, blur_size, inc_upper_bound=np.pi / 8,
                                                       inc_range=np.pi / 64)
    ...
    inclination = np.pad(inclination[:, :, np.newaxis], ((0, 0), (0, 0), (50, 49)), mode='constant')
else:
...
epsilon_tensor[1,1] = epsilon_mean - epsilon_del*(np.cos(inclination)**2 + np.sin(inclination)**2 * np.cos(2*azimuth))
...
del_f_component[0] = np.real(((2*np.pi/lambda_illu)**2)*(n_media**2 -  epsilon_mean + epsilon_del*np.cos(inclination)**2))
del_f_component[1] = np.imag(((2*np.pi/lambda_illu)**2)*(n_media**2 -  epsilon_mean + epsilon_del*np.cos(inclination)**2))
...
del_f_component[6] = ((2*np.pi/lambda_illu)**2) * epsilon_del * (np.sin(inclination)**2 - 2*np.cos(inclination)**2)
...
I_meas_SEAGLE, Stokes_SEAGLE = simulator.simulate_3D_vectorial_measurements_SEAGLE(epsilon_tensor, itr_max = 0, tolerance=1e-4, verbose=True)
...
np.savez(output_dir+output_file, I_meas=I_meas_noise, lambda_illu=lambda_illu, n_media=n_media, NA_obj=NA_obj, NA_illu=NA_illu,\
