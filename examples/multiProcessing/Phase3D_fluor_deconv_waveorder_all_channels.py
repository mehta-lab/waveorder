...
# 2) Modify N_pos to the number of positions you want to process (if more than the number of processes, this script will split the positions evenly across the processes 
#
# A typical workflow looks like:
# >> ssh <user.name>@login01.czbiohub.org
# >> cd /location/of/this/script/
# >> srun --partition=cpu --cpus-per-task=<ENTER-N_processes-here> --mem-per-cpu=8G ./Phase3D_fluor_deconv_waveorder_all_channels.py
#
# Debugging & profiling:
# The current script runs a short job (~20 seconds) for testing purposes. Modify or comment this lines to run a complete job.
# N_processes = 20
# N, M = 10, 10
# N_position = 20
...
N_position = 20 # Optional truncate number of positions for testing
...
# Process and save data to the assigned location
flu_reg = [1e-1, 1e-1, 1e-1]
bg = [50, 50, 50]
phase_reg = 1e-1
