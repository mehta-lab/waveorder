...
data_path = "/gpfs/CompMicro/projects/waveorderData/data_processing/20200909_cardiomyocyte_100x_14_147_oil_LCD/Low_MOI_FOV1/"
output_path = "/gpfs/CompMicro/projects/waveorderData/data_processing/20200909_cardiomyocyte_100x_14_147_oil_LCD/Low_MOI_FOV1/Full_FOV_process_test_new_pipeline_script"  # output image path

use_gpu = True
gpu_id = 2

reg_inc = (
    np.array([2.5, 2.5, 40, 40, 80, 80, 40]) * 1
...
PTI_file_name = data_path + "Cardiomyocytes_infected_1_raw.zarr"
reader = WaveorderReader(PTI_file_name, "zarr")
I_meas_full = np.transpose(reader.get_array(0), (0, 1, 3, 4, 2))
I_bg_full = np.squeeze(np.transpose(reader.get_array(1), (0, 1, 3, 4, 2)))

# Load calibration
PTI_file = zarr.open(PTI_file_name, mode="a")
...
I_ph /= np.max(I_ph)
for i, array in enumerate([I_ph, I_ret, I_ret]):
    I_render[:, :, :, i] = array.copy() * 255
I_render = np.uint8(I_render)

fig, ax = plt.subplots(1, 2, figsize=(10, 5))
sub_ax = ax[0].imshow(I_render[:, :, z_layer], origin="lower")
sub_ax = ax[1].imshow(
    np.transpose(I_render[50, :, :], (1, 0, 2)),
    origin="lower",
    aspect=z_step / ps,
)
plt.show()
...
# Load fluorescence data
fluor_file_name = data_path + "Cardiomyocytes_infected_1_fluor_decon.zarr"
...
I_rgb[:, :, :, 1] = np.clip(
    (I_fluor[1]) ** 0.8 / np.max((I_fluor[1]) ** 0.8) * 255 * 6, 0, 255
)
I_rgb[:, :, :, 2] = np.clip(
    I_fluor[0] / np.max(I_fluor[0]) * 255 * 1.5, 0, 255
)
I_rgb[:, :, :, :] += np.tile(
    np.clip(I_fluor[2] / np.max(I_fluor[2]) * 255 * 3, 0, 255)[
        :, :, :, np.newaxis
    ],
    (1, 1, 1, 3),
)
...
plt.figure(figsize=(15, 15))
plt.imshow(
    np.transpose(I_rgb[y_layer], (1, 0, 2)), origin="lower", aspect=z_step / ps
)
plt.show()
