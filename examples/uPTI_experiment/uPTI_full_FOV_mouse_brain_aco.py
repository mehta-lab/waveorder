...
data_path     = '/gpfs/CompMicro/rawdata/hummingbird/LiHao/20200223_LCD_63x_147_oil_test/Mouse_brain_702_ROI2/data_2/'        # data path
calibration_path = '/gpfs/CompMicro/projects/waveorderData/data_processing/20200223_LCD_63x_147_oil_test/Mouse_brain_702_ROI2_2/'
bg_path       = '/gpfs/CompMicro/rawdata/hummingbird/LiHao/20200223_LCD_63x_147_oil_test/Mouse_brain_702_ROI1/bg_2/'           # background data path
output_path   = '/gpfs/CompMicro/projects/waveorderData/data_processing/20200223_LCD_63x_147_oil_test/Mouse_brain_702_ROI2_2/Full_FOV_process_test_new_pipeline_script'      # output image path
use_gpu       = True
gpu_id        = 0
...
for i in range(N_pattern):
    ...
    for ll in range(4): I_bg_full[ll,i,:,:] = I_bg_temp[start_idx[ll][0]::2,start_idx[ll][1]::2]
    for p in range(N_defocus):
        idx = N_defocus*i+p
        I_meas_temp = (plt.imread(files[idx]).astype('float64'))**gamma_comp
        for ll in range(4): I_meas_full[ll,i,:,:,p] = I_meas_temp[start_idx[ll][0]::2,start_idx[ll][1]::2]
...
for ll in range(len(ns)):
    ...
    S_image_tm[1] = S_image_recon[1] / S_bg_recon[0, :, :, :, np.newaxis] - S_bg_recon[1, :, :, :, np.newaxis] * \
                    S_image_recon[0] / S_bg_recon[0, :, :, :, np.newaxis] ** 2
    S_image_tm[2] = S_image_recon[2] / S_bg_recon[0, :, :, :, np.newaxis] - S_bg_recon[2, :, :, :, np.newaxis] * \
                    S_image_recon[0] / S_bg_recon[0, :, :, :, np.newaxis] ** 2
    ...
    _, _, _, mat_map = setup.scattering_potential_tensor_to_3D_orientation(f_tensor, S_image_tm,
                                                                           ...
                                                                           reg_ret_pr=reg_ret_pr, itr=60, step_size=0.3,
                                                                           fast_gpu_mode=True)

    uPTI_array = np.transpose(np.concatenate((f_tensor, mat_map), axis=0)[np.newaxis, ...],
...
retardance_pr_PT = np.array([((-1)**i)*wo.wavelet_softThreshold(((-1)**i)*retardance_pr_PT[i], 'db8', 0.004, level=1) for i in range(2)])
...
sub_ax = ax[0,1].imshow(np.transpose(phase_crop[:,115,:]), cmap='gray', origin='lower',vmin=phase_min, vmax=phase_max, aspect=z_step/ps)
...
sub_ax = ax[1,1].imshow(np.transpose(ret_crop[:,115,:]), cmap='gray', origin='lower',vmin=ret_min, vmax=ret_max, aspect=z_step/ps)
plt.colorbar(sub_ax, ax=ax[1,1])
plt.show()

# structural orientation from the phase channel

VOI = np.abs(phase_crop-np.min(phase)).copy()
scale = [0.1]
local_az, local_th, seg_map, kernel = wo.cylindrical_shell_local_orientation(VOI, ps, z_step, scale, beta=0.5, c_para=0.5,evec_idx = 2)
...
az = np.array([local_az.flatten(), azimuth_crop.flatten()])
th = np.array([local_th.flatten(), theta_crop.flatten()])
val = np.array([seg_map.flatten(), np.abs(ret_crop*mask).flatten()])
wo.orientation_3D_hist(az, th, val, bins=36, num_col=2, size=5, contour_level = 100, hist_cmap='gnuplot2',top_hemi=True, colorbar=False)
plt.show()
