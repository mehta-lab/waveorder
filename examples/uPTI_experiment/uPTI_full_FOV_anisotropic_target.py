...
for ll in range(len(ns)):
    ...
    _, _, _, mat_map = setup.scattering_potential_tensor_to_3D_orientation(f_tensor, S_image_tm,
                                                                           ...
                                                                           reg_ret_pr=1e-1, itr=20, step_size=0.3,
                                                                           fast_gpu_mode=True)
...
sub_ax = ax[4].imshow(np.abs(retardance_pr_PT[1,:,:,z_layer]), cmap='gray', origin='lower',vmin=ret_min, vmax=ret_max)
plt.colorbar(sub_ax, ax=ax[4])
sub_ax = ax[5].imshow(np.transpose(np.abs(retardance_pr_PT[1,y_layer,:,:])), cmap='gray', origin='lower',vmin=ret_min, vmax=ret_max, aspect=z_step/ps)
