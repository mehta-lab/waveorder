...
@click.command()
@click.option(
    "--zarr_path", required=True, type=str, help="path to the save directory"
)
@click.option(
    "--raw_stats_path",
    required=True,
    type=str,
    help="path to the statistics file of the raw data",
)
        ...
        for t in range(shape[1]):
            for c in range(shape[2]):
                for z in range(shape[3]):
                    ...
                    std = np.std(image)
                    file.write(
                        f"Coord: {(p, t, c, z)}, Mean: {mean}, Median: {median}, Std: {std}\n"
                    )
