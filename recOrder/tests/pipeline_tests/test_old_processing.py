...
def test_old_processing(get_ometiff_data_dir, setup_data_save_folder):
    ...
    # Check Stokes
    assert (
        np.sum(np.abs(stokes[0, z, :, :] - array[0, 0, z]) ** 2)
        / np.sum(np.abs(stokes[0, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes[1, z, :, :] - array[0, 1, z]) ** 2)
        / np.sum(np.abs(stokes[1, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes[2, z, :, :] - array[0, 2, z]) ** 2)
        / np.sum(np.abs(stokes[2, z, :, :])) ** 2
        < 0.1
    )
    assert (
        np.sum(np.abs(stokes[3, z, :, :] - array[0, 3, z]) ** 2)
