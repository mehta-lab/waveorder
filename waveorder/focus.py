...
def compute_midband_power(
    ...
    if input_tensor.ndim == 2:
        ...
        raise ValueError(
            f"{input_tensor.ndim}D tensor supplied. `compute_midband_power` only accepts 2D or 3D tensors."
        )
...
def focus_from_transverse_band(
    ...
    midband_sum = (
        compute_midband_power(zyx_tensor, NA_det, lambda_ill, pixel_size, midband_fractions).cpu().numpy()
    )  # (Z,)
