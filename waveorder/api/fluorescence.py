...
def simulate(
    ...
    if recon_dim == 3:
        ...
        zyx_data = isotropic_fluorescent_thick_3d.apply_transfer_function(
            zyx_fluorescence, otf, z_padding=0
        )
...
def compute_transfer_function(
    ...
    if recon_dim == 2:
        ...
        return xr.Dataset(
            {
                "optical_transfer_function": _named_dataarray(
                    optical_tf.cpu().numpy(), "optical_transfer_function"
                ),
            }
        )
...
def reconstruct(
    ...
    return apply_inverse_transfer_function(
        czyx_data, tf, recon_dim, settings, fluor_channel_name
    )
