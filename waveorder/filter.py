...
def apply_filter_bank(
    ...
    # Matrix-vector multiplication over f — batched over all i/o channels at once
    padded_output_spectrum = stretched_matrix_multiply(
        io_filter_bank, padded_input_spectrum
    )
    ...
    slices = (slice(None),) + tuple(
        slice(0, s) for s in i_input_array.shape[1:]
    )
...
def stretched_matrix_multiply(
    ...
    """
    Batched version of stretched_multiply over input/output channels.

    Equivalent to:
        for i in range(I):
            for o in range(O):
                out[o] += stretched_multiply(io_filter_bank[i, o], spectrum[i])
    ...
    padded_input_spectrum : torch.Tensor
        Shape (I, l0, l1, ..., l_{n-1}), where each lk is divisible by sk.
    ...
    spectrum_reshaped = padded_input_spectrum.reshape(
        (num_input_channels, 1) + interleaved_l
    )

    # (I, O, s0, ...) -> (I, O, s0, 1, s1, 1, ...)
    interleaved_s = tuple(itertools.chain(*zip(s_shape, ndim * (1,))))
    filter_reshaped = io_filter_bank.reshape(
        (num_input_channels, num_output_channels) + interleaved_s
    )
