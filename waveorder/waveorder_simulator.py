...
class waveorder_microscopy_simulator:
    ...
    def illumination_setup(self, illu_mode, NA_illu_in, Source, Source_PolState):
        if illu_mode == "BF":
            self.Source = generate_pupil(self.frr, self.NA_illu, self.lambda_illu).numpy()
            self.N_pattern = 1
            ...
            for j in range(self.N_pattern):
                ...
                for i in range(N_source):
                    plane_wave = Source_current[idx_y[i], idx_x[i]] * np.exp(
                        ...
                        * 2
                        * np.pi
                        * (self.fyy[idx_y[i], idx_x[i]] * self.yy + self.fxx[idx_y[i], idx_x[i]] * self.xx)
                    )
                    E_field = []
                    E_field.append(plane_wave)
                    E_field.append(1j * plane_wave)  # RHC illumination
    ...
    def simulate_waveorder_inc_measurements(self, n_e, n_o, dz, mu, orientation, inclination):
        ...
        for j in range(self.N_pattern):
            ...
            for i in range(N_source):
                ...
                plane_wave = Source_current[idx_y[i], idx_x[i]] * np.exp(
                    1j * 2 * np.pi * (self.fyy[idx_y[i], idx_x[i]] * self.yy + self.fxx[idx_y[i], idx_x[i]] * self.xx)
                )
                E_field = []
                E_field.append(plane_wave)
                E_field.append(1j * plane_wave)  # RHC illumination
    ...
    def simulate_3D_scalar_measurements(self, t_obj):
        ...
        Hz_defocus = Pupil_prop[:, :, np.newaxis] * np.exp(
            1j * 2 * np.pi * z_defocus[np.newaxis, np.newaxis, :] * oblique_factor_prop[:, :, np.newaxis]
        )
                ...
                for j in range(N_pt_source):
                    plane_wave = cp.array(
                        ...
                        * np.exp(
                            ...
                            * 2
                            * np.pi
                            * (self.fyy[idx_y[j], idx_x[j]] * self.yy + self.fxx[idx_y[j], idx_x[j]] * self.xx)
                        )
                    )
                    ...
                    for m in range(self.N_defocus):
                        ...
                        if m == self.N_defocus - 1:
                            f_field_stack_f = cp.fft.fft2(g_field[:, :, cp.newaxis], axes=(0, 1)) * Hz_defocus
                            I_temp += (
                                cp.abs(
                                    cp.fft.ifft2(
                                        f_field_stack_f * Pupil_obj[:, :, cp.newaxis],
                                        axes=(0, 1),
                                    )
                                )
                                ** 2
                            ...
                            f_field = cp.fft.ifft2(cp.fft.fft2(g_field) * Hz_step)
                    ...
                    for m in range(self.N_defocus):
                        ...
                        if m == self.N_defocus - 1:
                            ...
                            I_meas[i] += (
                                np.abs(
                                    ifft2(
                                        f_field_stack_f * self.Pupil_obj[:, :, np.newaxis],
                                        axes=(0, 1),
                                    )
                                )
                                ** 2
    ...
    def simulate_3D_scalar_measurements_SEAGLE(self, RI_map, itr_max=100, tolerance=1e-4, verbose=False):
        ...
        Hz_defocus = Pupil_prop[:, :, np.newaxis] * np.exp(
            1j * 2 * np.pi * z_defocus_m[np.newaxis, np.newaxis, :] * oblique_factor_prop[:, :, np.newaxis]
        )
            ...
            if self.use_gpu:
                ...
                for j in range(N_pt_source):
                    ...
                    for m in range(itr_max):
                        ...
                        step_size = cp.sum(cp.abs(grad_u) ** 2) / cp.sum(cp.abs(A_grad_u) ** 2)
                    ...
                    I_temp += (
                        cp.abs(
                            cp.fft.ifft2(
                                cp.fft.fft2(u[:, :, -1])[:, :, cp.newaxis] * Pupil_obj[:, :, cp.newaxis] * Hz_defocus,
                                axes=(0, 1),
                            )
                        )
                        ** 2
                ...
                for j in range(N_pt_source):
                    ...
                    I_meas[i] += (
                        np.abs(
                            ifft2(
                                fft2(u[:, :, -1])[:, :, np.newaxis] * self.Pupil_obj[:, :, np.newaxis] * Hz_defocus,
                                axes=(0, 1),
                            )
                        )
                        ** 2
            ...
            for j in range(N_pt_source):
                ...
                if fr[idx_y[j], idx_x[j]] == 0:
                    ...
                    E_in_amp[0] = (
                        ...
                        * (
                            ...
                            * (1 - self.lambda_illu**2 * fr[idx_y[j], idx_x[j]] ** 2) ** (1 / 2)
                    ...
                    E_in_amp[2] = -self.lambda_illu * (
                        self.Source_PolState[i, 0] * self.fxx[idx_y[j], idx_x[j]]
                ...
                for p in range(3):
                    E_in[p] = (
                        E_in_amp[p]
                        * np.exp(
                            1j * 2 * np.pi * (self.fyy[idx_y[j], idx_x[j]] * yy + self.fxx[idx_y[j], idx_x[j]] * xx)
                        )[:, :, np.newaxis]
                        * np.exp(
                            1j
                            * 2
                ...
                if self.use_gpu:
                    E_field_out = cp.fft.ifft2(
                        cp.fft.fft2(E_tot[:2, :, :, -1], axes=(1, 2))[:, :, :, cp.newaxis]
                        * (Pupil_obj[:, :, cp.newaxis] * Hz_defocus)[cp.newaxis, :, :, :],
                        axes=(1, 2),
                    )
                    ...
                    E_field_out = ifft2(
                        fft2(E_tot[:2, :, :, -1], axes=(1, 2))[:, :, :, np.newaxis]
                        * (self.Pupil_obj[:, :, np.newaxis] * Hz_defocus)[np.newaxis, :, :, :],
                        axes=(1, 2),
                    )
